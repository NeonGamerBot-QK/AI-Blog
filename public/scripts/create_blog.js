...
(async () => {
      ...
      messages: [
        {
          role: "system",
          content:
            "Create an AI article on any topic that is SFW. Respond in JSON. Schema the json with properties artcle which is the artcle content structured in paragraphs and titles { content: string; title: string}[] . tags which is an array with tags related to the article.  Title related to the article.  topic which tells me what the topic of the article is. No code block.",
        },
      ],
      model: "gpt-4o",
    })
  ...
  if (process.env.TESTING || process.env.NO_IMAGE) {
